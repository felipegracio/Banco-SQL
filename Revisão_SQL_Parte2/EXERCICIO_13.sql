...
SELECT ca.marca, ca.modelo, SUM(vi.distanciaPercorrida) AS Km
FROM carro ca, viagem vi
WHERE ca.id = vi.idCarro
GROUP BY ca.marca, ca.modelo

--2)Consultar Nomes das empresas que não tem carros cadastrados
SELECT em.nome
FROM empresa em LEFT OUTER JOIN carro ca
ON em.id = ca.idEmpresa
WHERE ca.idEmpresa IS NULL

--Consultar Marca e modelos dos carros que não fizeram viagens
SELECT ca.marca, ca.modelo
FROM carro ca LEFT OUTER JOIN viagem vi 
ON ca.id = vi.idCarro
WHERE vi.idCarro IS NULL
...
SELECT em.nome,ca.marca, ca.modelo, COUNT(vi.idCarro) AS Qtd_viagens
FROM carro ca, viagem vi, empresa em
WHERE ca.id = vi.idCarro
AND em.id = ca.idEmpresa
GROUP BY ca.marca, ca.modelo, em.nome
ORDER BY em.nome asc, COUNT(vi.idCarro)desc 
...
SELECT em.nome, ca.marca, ca.modelo, 
CASE	
	WHEN vi.distanciaPercorrida < 1000 THEN
		CAST(vi.distanciaPercorrida*10 AS DECIMAL(7,2)) 
		WHEN vi.distanciaPercorrida > 1000 THEN
			CAST(vi.distanciaPercorrida*15 AS DECIMAL(7,2)) 
			END AS Valor_Viagem

FROM empresa em, carro ca, viagem vi
WHERE ca.id = vi.idCarro
AND em.id = ca.idEmpresa
ORDER BY em.nome, ca.marca, ca.modelo, vi.distanciaPercorrida