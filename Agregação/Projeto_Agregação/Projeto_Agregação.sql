...
WHERE descrição LIKE 'Manutenção%'
...
-- Consultar: Id do projeto, nome do projeto, qty_users_project
-- (quantidade de usuários por projeto) em ordem alfabética crescente
-- pelo nome do projeto


SELECT proj.id, proj.nome, COUNT(us_proj.users_id) AS qty_users_project
FROM PROJECTS proj, USERS_HAS_PROJECT us_proj
WHERE proj.id = us_proj.projects_id
GROUP BY proj.id, proj.nome
ORDER BY proj.nome
