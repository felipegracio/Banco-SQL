...
SELECT TOP 1 cl.num_Cadastro, cl.nome, fil.titulo,(dvd.data_fabricação) AS data_Fabricação, loc.valor
FROM CLIENTE cl, LOCACAO loc, FILME fil, DVD dvd
WHERE cl.num_Cadastro = loc.Clientenum_Cadastro
AND  loc.DVDnum = dvd.num
AND  dvd.FilmeId = fil.id
GROUP BY cl.num_Cadastro, cl.nome, fil.titulo, dvd.data_fabricação, loc.valor
ORDER BY dvd.data_fabricação desc
...
SELECT  cl.num_Cadastro, cl.nome, CONVERT(CHAR(10), loc.data_locação, 103) AS data_Locacao, COUNT(loc.DVDnum) AS qtd
FROM CLIENTE cl, LOCACAO loc
WHERE cl.num_Cadastro = loc.Clientenum_Cadastro
GROUP BY cl.num_Cadastro, cl.nome, loc.data_locação
...
SELECT cl.num_Cadastro, cl.nome, CONVERT(CHAR(10), loc.data_locação, 103) AS data_Locacao, SUM(loc.valor) AS valor_Total
FROM CLIENTE cl, LOCACAO loc, DVD dvd
WHERE cl.num_Cadastro = loc.Clientenum_Cadastro
...
SELECT cl.num_Cadastro, cl.nome, cl.logradouro + ' ' + CAST(cl.num AS VARCHAR(5)) + ' , ' + SUBSTRING(cl.cep, 1, 5) +
'-'+SUBSTRING(cl.cep,6,3)  AS Endereço_Completo, CONVERT(CHAR(10), loc.data_locação, 103) AS data_Locacao
FROM CLIENTE cl, LOCACAO loc, DVD dvd
WHERE cl.num_Cadastro = loc.Clientenum_Cadastro
      AND dvd.num = loc.DVDnum
GROUP BY cl.num_Cadastro, cl.nome, cl.logradouro, cl.num, cl.cep, loc.data_locação
HAVING COUNT(loc.DVDnum)>1
